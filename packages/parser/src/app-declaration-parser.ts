...
function parse({
  ...
  if (node.kind === ts.SyntaxKind.NumberKeyword) {
    ...
    // if there is a typeName, check if there is a reference for this type
    if (typeName) {
    ...
    // todo: check if appendType is really needed here
    // return appendType({
    //   context,
    //   replaceReferencesOnParentDeepnessLevel,
    //   parentName,
    //   parentType,
    //   type: TypeMetadataUtils.create("object", {
    //     propertyName,
    //     propertyPath: ParserUtils.joinStrings(parentName, propertyName),
    //   }),
    // })
    ...
    const allAllBigIntLiterals = typesWithoutNullAndUndefined.every((type) => {
    ...
    if (allAllStringLiterals) {
      ...
      //   parentName,
      //   parentType,
      //   type: TypeMetadataUtils.create("enum", {
      //     // typeName,
      //     nullable,
      //     canBeUndefined,
    ...
    const missingPropertiesInTypes = properties.filter((metadata) => {
      ...
      if (!context.typeNames.includes(metadata.typeName)) return true
    ...
    //   parentName,
    //   parentType,
    //   type: TypeMetadataUtils.create("union", {
    //     // typeName,
    //     nullable,
    //     canBeUndefined,
    //     properties,
    ...
    if (!type.typeName && context.typeNames.includes(node.name.text)) {
    ...
    if (
      ...
      if (!modelType) throw Errors.invalidModelSignature(modelName, parentName)
    ...
    if (context.typeNames.includes(typeName)) {
    ...
    const modelSymbol = context.program
      .getTypeChecker()
  ...
  for (let member of members) {
    ...
    if (ts.isEnumMember(member)) {
      ...
      if (propertyName !== parsed.propertyName) {
        throw Errors.enumMemberNameMismatch(parentName, propertyName, debugPath)
      }
...
function findReference({
  ...
  const existType = context.typeNames.find((type) => type === typeName)
  ...
  const deepnessCheck = ParserUtils.checkPathDeepness(parentName, {
    regular: replaceReferencesOnParentDeepnessLevel,
    args: 1,
  })
