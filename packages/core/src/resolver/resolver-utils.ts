...
export const ResolverUtils = {
     ...
     */
    normalizeResolverMetadatas(givenResolvers: ListOfType<AppResolverType>): ResolverMetadata[] {
        const resolvers: ResolverMetadata[] = []
        if (givenResolvers instanceof Array) {
            resolvers.push(...givenResolvers.map(resolver => {
                if (resolver instanceof Function) {
                    return resolver.prototype.resolver

                } else if (resolver instanceof Object && !this.isResolverMetadata(resolver)) {
                    return {
                        instanceof: "Resolver",
            ...
            resolvers.push(...Object.keys(givenResolvers).map(key => {
