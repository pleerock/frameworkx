...
// const ReconnectingWebSocketCls =
...
export class Fetcher {
  ...
  async fetch<T>(
    ...
    // const accountId = window.localStorage
    //   ? window.localStorage.getItem("account")
    //   : null
    // if (accountId) {
    //   headers["Authorization"] =
    //     "Basic " + window.localStorage.getItem("account")
    // }

    const res = await fetch(this.options.graphqlEndpoint + "?" + queryName, {
      method: "POST",
      headers: headers,
      body: JSON.stringify({
        query: queryString,
    ...
    if (result["errors"]) {
      throw new Error(
        `GraphQL(${queryName}): ${JSON.stringify(result["errors"])}`,
      )
    }
    return result // .data
  }
  ...
  subscription<T>(
    ...
    if (typeof query === "string") {
      queryString = query
    } else if (query["typeof"] === "Request") {
      queryName = query.name
      queryString = this.requestToQuery(query as Request<any>)
      dataName = Object.keys(query.map)[0]
    } else {
      ...
      const onMessageCallback = (event: any) => {
        ...
        if (data.payload) {
          ...
          if (data.payload.data) {
            ...
            if (!dataName) dataName = Object.keys(data.payload.data)[0]
