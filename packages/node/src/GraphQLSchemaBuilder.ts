...
export class GraphQLSchemaBuilder {
     ...
     */
    canHaveSchema() {
        return  Object.keys(this.appMetadata.queries).length > 0 ||
    ...
    build(): GraphQLSchema {
        ...
        for (let model of this.appMetadata.models) { // todo: can't we simply call resolveGraphQLType method here?
            if (model.kind === "enum") {
        ...
        for (let input of this.appMetadata.inputs) { // todo: can't we simply call resolveGraphQLType method here?
            this.createGraphQLInputType(input)
        }

        return new GraphQLSchema({
            types: this.objectTypes,
            query: this.createRootGraphQLObjectType("query", this.appMetadata.queries),
            mutation: this.createRootGraphQLObjectType("mutation", this.appMetadata.mutations),
            subscription: this.createRootGraphQLObjectType("subscription", this.appMetadata.subscriptions),
        })
    }
     ...
     */
    private createGraphQLInputType(metadata: TypeMetadata): GraphQLInputObjectType {

        // check if we already have a type with such name
        const existType = this.inputTypes.find(inputType => inputType.name === metadata.typeName)
        if (existType)
            return existType
    ...
    private createGraphQLEnumType(metadata: TypeMetadata): GraphQLEnumType {
        ...
        const existEnum = this.enumTypes.find(type => type.name === metadata.typeName)
        if (existEnum)
            return existEnum
    ...
    private createGraphQLUnionType(metadata: TypeMetadata): GraphQLUnionType {
        ...
        const existUnion = this.unionTypes.find(type => type.name === metadata.typeName)
        if (existUnion)
            return existUnion
        ...
        const newUnion = new GraphQLUnionType({
            ...
            types: () => {
                return metadata.properties.map(property => {
                    const type = this.objectTypes.find(type => type.name === property.typeName)
                    if (!type)
            ...
            fields: () => {
                ...
                for (const property of metadata.properties) {
                    ...
                    fields[property.propertyName] = {
                        ...
                        description: property.description,
                        resolve: this.createRootDeclarationResolverFn("model", property, typeName!!)
                    }
                    if (property.args) {
    ...
    private createRootGraphQLObjectType(
        ...
        const description = this.properties.namingStrategy.defaultTypeDescription(type)
        const fields: GraphQLFieldConfigMap<any, any> = {}
        for (let metadata of metadatas) {
            ...
            if (type === "subscription") {
                let subscriptionResolverFn = this.resolverHelper.findSubscriptionResolver(metadata.propertyName)

                if (subscriptionResolverFn) {
                    fields[metadata.propertyName].subscribe = this.resolverHelper.createSubscribeResolver({
    ...
    private resolveGraphQLTypeBasedOnTypeReference(metadata: TypeMetadata) {
        ...
        if (!metadata.typeName && metadata.modelName) {
            ...
            if (metadataModel) {
                return this.resolveGraphQLType("object", { ...metadata, typeName: metadataModel.typeName })
            }
        } if (metadata.typeName) {
        ...
        throw new Error(`Cannot resolve GraphQL type for ${JSON.stringify(metadata)}`)
    }
    ...
    private createRootDeclarationResolverFn(
        ...
        if (type === "model" && parentTypeName) {
            ...
            const dataLoaderResolverFn = this.resolverHelper.findDataLoaderResolver(parentTypeName, metadata.propertyName)
            if (dataLoaderResolverFn) {
                return this.resolverHelper.createGraphQLTypeDataLoaderResolver(metadata, dataLoaderResolverFn, parentTypeName)
            }

            // try generated relation resolver if we have a data source setup
            if (this.properties.dataSource &&  this.properties.dataSource.hasMetadata(parentTypeName)) {
                const entityRelation = this.properties.dataSource
                    .getMetadata(parentTypeName)
                    .relations
                    .find(relation => relation.propertyName === metadata.propertyName!!)
                if (entityRelation) {
                    const entityRelationResolverFn = (parents: any[]) => {
                        return this.properties.dataSource!
                            .relationIdLoader
                            .loadManyToManyRelationIdsAndGroup(entityRelation, parents)
                            .then(groups => groups.map(group => group.related))
                    }
                    return this.resolverHelper.createGraphQLTypeGeneratedRelationResolver(metadata, entityRelationResolverFn, parentTypeName)
                }
            }
        }
